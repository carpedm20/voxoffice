...
j = json.loads(open('static/people.json').read())
...
def poster_url(code):
    b = BeautifulSoup(requests.get('http://movie.naver.com/movie/bi/pi/basic.nhn?st=1&code='+str(code)).text)
    try:
        img = b.findAll('div', {'class':'poster'})[0].find('img')
        src = img.attrs['src']
        src = src[src.find('&q=')+3:]
        print src, img.attrs['alt']
        return [src, img.attrs['alt']]
    except:
        return ['http://static.naver.net/movie/2012/06/dft_img111x139.png','']
...
for year in years:
    ...
    with open('people-%s.json' % (year),'w') as f:
        json.dump(ans, f)
